...
Léa (persona Rapidité) ouvre l'application web sur son ordinateur, commence immédiatement à taper une URL dans l'éditeur plein écran qui s'affiche, et voit un QR code généré en temps réel au fur et à mesure de sa saisie. Elle scanne le code avec son téléphone et accède instantanément à l'URL.
...
1. **Given** l'utilisateur ouvre l'application web, **When** la page se charge, **Then** un éditeur de texte plein écran apparaît avec le focus automatique en moins de 500ms
...
Un utilisateur doit transférer un long texte (article, code, configuration) vers son téléphone. Il colle le texte dans l'éditeur, et le système génère automatiquement un QR code adaptatif : pour les textes courts, un QR code compact ; pour les textes longs (> 2000 caractères), un rendu optimisé avec indication de la taille.

**Why this priority**: Étend l'utilité au-delà des URLs courtes. Permet des cas d'usage variés (secrets, configurations, snippets de code) tout en maintenant la compatibilité universelle.

**Independent Test**: Peut être testé en collant différentes longueurs de texte et en vérifiant que le QR code reste scannable. Fonctionne indépendamment de US1.
...
2. **Given** le texte dépasse 2000 caractères, **When** le QR code est généré, **Then** un niveau de correction d'erreur adapté est utilisé et la taille du QR est optimisée pour rester scannable
3. **Given** un texte très long (> 5000 caractères), **When** le QR code est généré, **Then** l'utilisateur voit une indication visuelle de la complexité du code et une recommandation de réduire le texte si nécessaire

---

### User Story 3 - Mode sombre adaptatif (Priority: P3)

Un utilisateur travaille dans un environnement sombre (bureau la nuit, transport en commun) et ouvre l'application. Le système détecte automatiquement la préférence de thème sombre du navigateur et affiche l'interface avec un fond noir et un QR code avec contraste optimisé (fond blanc, modules noirs) pour garantir la scannabilité.

**Why this priority**: Améliore l'expérience utilisateur et la scannabilité dans différentes conditions d'éclairage. Répond au pilier Universal Scannability en optimisant le contraste.

**Independent Test**: Peut être testé en changeant les préférences système (thème clair/sombre) et en vérifiant que l'interface s'adapte. Le QR code reste scannable dans les deux modes.

**Acceptance Scenarios**:

1. **Given** le système d'exploitation de l'utilisateur est en mode sombre, **When** l'application se charge, **Then** l'interface s'affiche avec un thème sombre (fond noir, texte blanc)
2. **Given** l'application est en mode sombre, **When** un QR code est généré, **Then** le code a un fond blanc et des modules noirs pour maximiser le contraste
3. **Given** l'utilisateur change la préférence de thème en cours d'utilisation, **When** le navigateur détecte le changement, **Then** l'interface bascule automatiquement entre mode clair et sombre sans perte de données

---

### User Story 4 - Effacement sécurisé (Priority: P4)
...
- **Texte extrêmement long (> 10 000 caractères)** : Le système doit afficher un avertissement indiquant que le QR code peut être difficile à scanner et suggérer de réduire le texte. Le QR code généré utilise le niveau de correction d'erreur minimum (Level L) pour minimiser la complexité.
...
- **FR-001**: System MUST provide a full-screen text editor accessible immediately upon page load (< 500ms Time-to-Interactive)
...
- **FR-010**: All QR code generation and text processing MUST execute client-side only (in-browser JavaScript)
...
- **FR-018**: System MUST display visual feedback when text exceeds recommended limits (> 5000 characters)
